...
for scale in [20, 50, 100, 150]:
    ...
    # Reshape the flat NDVI array back to the image shape
    ndvi_result = ndvi_result_flat.reshape(red_band.shape)
    
    # Save the NDVI image for the current scaling factor
    # filename = f"ndvi_gpu_scale_{scale}.png"
    # plt.imsave(filename, ndvi_result, cmap='gray')
    # print(f"Saved NDVI image as {filename}\n")
